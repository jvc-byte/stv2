...
const ShippingMethodDropdown = ({ value, onChange }: ShippingMethodDropdownProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement | null>(null);

  const shippingMethod = [
    { name: 'Cargo Shipping'},{ name: 'Air Shipping'},{ name: 'Standard Shipping'},{ name: 'Sea Shipping'},{ name: 'Road Shipping'},{ name: 'International Shipping'},
    { name: 'Local Shipping'},{ name: 'Express Shipping'},{ name: 'Same Day Shipping'},{ name: 'No Shipping'},
  ];
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      >
        <span className="text-gray-500">{value || 'Select Shipping Method...'}</span>
